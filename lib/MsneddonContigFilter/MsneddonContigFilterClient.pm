package MsneddonContigFilter::MsneddonContigFilterClient;
...
MsneddonContigFilter::MsneddonContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => MsneddonContigFilter::MsneddonContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "MsneddonContigFilter.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "MsneddonContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MsneddonContigFilter::MsneddonContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "MsneddonContigFilter::MsneddonContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package MsneddonContigFilter::MsneddonContigFilterClient::RpcClient;
